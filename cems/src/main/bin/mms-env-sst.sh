...
export MMS_TEMP_DIR='/group_workspaces/cems2/esacci_sst/mms_new/temp'
...
wait_for_task_jobs_completion() {
    ...
    do
        ...
        if bjobs -P sst-cci | egrep -q "^$jobs\\>"
        then
            continue
        fi
...
submit_job() {
    ...
    bsubmit="bsub -R rusage[mem=16000] -M 16000000 -q short-serial -W 12:00 -P sst-cci -cwd ${MMS_INST} -oo ${MMS_LOG}/${jobname}.out -eo ${MMS_LOG}/${jobname}.err -J ${jobname} ${MMS_HOME}/bin/${command} ${@:3}"
