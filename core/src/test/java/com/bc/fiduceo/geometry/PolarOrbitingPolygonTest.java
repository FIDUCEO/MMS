...
public class PolarOrbitingPolygonTest {
    ...
    public void testConstructor() {
        final List<Point> pointList = Arrays.asList(PolarOrbitingPolygonTestData.TEST_POLYGON);
        final Geometry geometry = factory.createPolygon(pointList);
    ...
    public void testIsPointInPolygon() {
        ...
        for (Point point : PolarOrbitingPolygonTestData.TEST_POINTS_INSIDE) {
            assertTrue("lat=" + point.getLat() + " lon=" + point.getLon(), polygon.isPointInPolygon(point.getLat(), point.getLon()));
        }

        for (Point point : PolarOrbitingPolygonTestData.TEST_POINTS_OUTSIDE) {
            assertFalse("lat=" + point.getLat() + " lon=" + point.getLon(), polygon.isPointInPolygon(point.getLat(), point.getLon()));
        }
    }
    ...
    public void testIsPointInRing() {
        ...
        assertFalse(
                "lat=" + PolarOrbitingPolygonTestData.TEST_POINTS_INSIDE[0].getLat() + " lon=" + PolarOrbitingPolygonTestData.TEST_POINTS_INSIDE[0].getLon(),
                polygon.isPointInRing(PolarOrbitingPolygonTestData.TEST_POINTS_INSIDE[0].getLat(),
                        PolarOrbitingPolygonTestData.TEST_POINTS_INSIDE[0].getLon(), points));
        assertTrue(
                "lat=" + PolarOrbitingPolygonTestData.TEST_POINTS_INSIDE[1].getLat() + " lon=" + PolarOrbitingPolygonTestData.TEST_POINTS_INSIDE[1].getLon(),
                polygon.isPointInRing(PolarOrbitingPolygonTestData.TEST_POINTS_INSIDE[1].getLat(),
                        PolarOrbitingPolygonTestData.TEST_POINTS_INSIDE[1].getLon(), points));
        assertTrue(
                "lat=" + PolarOrbitingPolygonTestData.TEST_POINTS_INSIDE[2].getLat() + " lon=" + PolarOrbitingPolygonTestData.TEST_POINTS_INSIDE[2].getLon(),
                polygon.isPointInRing(PolarOrbitingPolygonTestData.TEST_POINTS_INSIDE[2].getLat(),
                        PolarOrbitingPolygonTestData.TEST_POINTS_INSIDE[2].getLon(), points));
    }
    ...
    public void testAtsr2Polygon() {
        ...
        final List<Point> pointList = Arrays.asList(PolarOrbitingPolygonTestData.ATSR2_POINTS);
        final Geometry geometry = factory.createPolygon(pointList);
        final PolarOrbitingPolygon polygon = new PolarOrbitingPolygon(1, System.currentTimeMillis(), geometry);
    ...
    public void testGetId() {
        final Polygon polygon = (Polygon) factory.parse("POLYGON((10 30, 10.5 30, 11 30, 11.5 30, 12 30, 12 30.5, 12 31, 12 31.5, 12 32, 11.5 32, 11 32, 10.5 32, 10 32, 10 31.5, 10 31, 10 30.5, 10 30))");
    ...
    @Test
    public void testRectangleInNorthernHemisphere() {
    ...
    @Test
    public void testRectangleInSouthernHemisphere() {
        // remember: WKT is (lon/lat) tb 2014-02-04
        final Polygon polygon = (Polygon) factory.parse("POLYGON((10 -50, 10.5 -50, 11 -50, 11.5 -50, 12 -50, 12 -50.5, 12 -51, 12 -51.5, 12 -52, 11.5 -52, 11 -52, 10.5 -52, 10 -52, 10 -51.5, 10 -51, 10 -50.5, 10 -50))");
        final PolarOrbitingPolygon polarOrbitingPolygon = new PolarOrbitingPolygon(12, 14, polygon);
        ...
        // upper left
        // @todo 1 tb/tb check why this fails 2016-01-27
    ...
    @Test
    public void testRhomboidOnEquator() {
        // remember: WKT is (lon/lat) tb 2014-02-05
        final Polygon polygon = (Polygon) factory.parse("POLYGON((-8 -2, -7.75 -1, -7.5 0, -7.25 1, -7 2, -6 2, -5 2, -4 2, -3 2, -3.25 1, -3.5 0, -3.75 -1, -4 -2, -5 -2, -6 -2, -7 -2, -8 -2))");
        final PolarOrbitingPolygon polarOrbitingPolygon = new PolarOrbitingPolygon(12, 14, polygon);
    ...
    public void testAvhrrPolygon() {
        final List<Point> pointList = Arrays.asList(PolarOrbitingPolygonTestData.AVHRR_POINTS);
        final Geometry geometry = factory.createPolygon(pointList);
        final PolarOrbitingPolygon polygon = new PolarOrbitingPolygon(1, System.currentTimeMillis(), geometry);
    ...
    @Test
    public void testAvhrrPolygon2() {
        final Geometry geometry = factory.parse(
...
//        final Geometry[] geometries = new Polygon[]{
//                // AVHRR-GAC
//                new Polygon(
...
//                // AMSR2
//                new Polygon(
//                        "POLYGON((-152.628768920898 82.5190200805664,60.9478950500488 88.2457275390625,35.8603134155273 79.4457931518555,32.5379981994629 70.481315612793,30.5094814300537 61.4996795654297,28.8221645355225 52.5048065185547,27.249059677124 43.496654510498,25.6950130462646 34.4775276184082,24.1254043579102 25.4477596282" +
//                                "959,22.4945507049561 16.4176864624023,20.7502632141113 7.3938627243042,18.8641319274902 -1.61513292789459,16.7860984802246 -10.5894985198975,14.4133176803589 -19.515007019043,11.6113367080688 -28.3673839569092,8.20943355560303 -37.1114845275879,3.84108376502991 -45.6924018859863,-2.14518189430237 -54.0141639709473,-1" +
//                                "1.0530366897583 -61.8681259155273,-25.5740604400635 -68.7550888061523,-50.0450477600098 -73.4579162597656,-54.3909530639648 -73.8252410888672,-77.7207183837891 -80.00927734375,-45.0763282775879 -87.4911727905273,11.6951189041138 -85.8399658203125,19.1855697631836 -76.9391326904297,21.7397041320801 -68.0049133300781,2" +
//                                "3.4704971313477 -59.0414199829102,25.0165424346924 -50.0629768371582,26.5146789550781 -41.0577812194824,28.0040950775146 -32.0412864685059,29.5533580780029 -23.0091495513916,31.1810913085938 -13.9825162887573,32.917896270752 -4.95595741271973,34.8292007446289 4.04986429214478,36.9478225708008 13.0236921310425,39.3984" +
//                                "489440918 21.9370231628418,42.2880439758301 30.7750110626221,45.8493003845215 39.5171012878418,50.5612449645996 48.0510139465332,57.134033203125 56.3146133422852,67.2142639160156 64.017204284668,84.1077575683594 70.5215072631836,112.137985229492 74.2862701416016,145.242965698242 73.3402709960938,149.285385131836 72.8" +
//                                "286056518555,135.004501342773 79.919563293457,-175.280364990234 84.9528579711914,-152.628768920898 82.5190200805664))")};
//
...
//    }
    ...
    private void rotate(Point[] points, double skip) {
        for (Point point : points) {
            ...
            point.setLon(normalizeLongitude(lon + skip));
        }
        assertFalse(PolarOrbitingPolygon.isEdgeCrossingEquator(1.0, 1.0));
        assertFalse(PolarOrbitingPolygon.isEdgeCrossingEquator(-1.0, -1.0));
    }
