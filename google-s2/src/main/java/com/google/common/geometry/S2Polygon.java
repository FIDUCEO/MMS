...
public final strictfp class S2Polygon implements S2Region, Comparable<S2Polygon> {
    ...
    public List<S2Polyline> intersectWithPolyLine(S2Polyline polyLine) {
        return intersectWithPolyLineSloppy(polyLine, S2EdgeUtil.DEFAULT_INTERSECTION_TOLERANCE);
    }

    public List<S2Polyline> intersectWithPolyLineSloppy(S2Polyline polyLine, S1Angle vertexMergeRadius) {
        return internalClipPolyline(false, polyLine, vertexMergeRadius);
    }
    ...
    List<S2Polyline> internalClipPolyline(boolean invert, S2Polyline polyline, S1Angle errorRadius) {
        final S2PolygonIndex s2PolygonIndex = new S2PolygonIndex(this, false);
        final List<ParametrizedS2Point> intersections = new ArrayList<>();
        final List<S2Point> vertices = new ArrayList<>();
        final List<S2Polyline> result = new ArrayList<>();
        int n = polyline.numVertices();

        boolean inside = contains(polyline.vertex(0)) ^ invert;
        for (int j = 0; j < n - 1; j++) {
            final S2Point a0 = polyline.vertex(j);
            final S2Point a1 = polyline.vertex(j + 1);

            clipEdge(a0, a1, s2PolygonIndex, true, intersections);
            if (inside) {
            ...
            for (int k = 0; k < intersections.size(); k += 2) {
                ...
                final S2Point p0 = paramPoint_0.getPoint();
                final S2Point p1 = paramPoint_1.getPoint();

                // If the gap from the previous vertex to this one is large
                // enough, start a new polyline.
                if (!vertices.isEmpty() && vertices.get(vertices.size() - 1).angle(p0) > errorRadius.radians()) {
